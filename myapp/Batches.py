...
@csrf_exempt
@api_view(['GET','POST'])
def saveBatchView(request):  #this service will save class room batch details
    #connect to our local mongodb
    db = Connection(settings.MONGO_SERVER_ADDR,settings.MONGO_PORT)
    #get a connection to our database
    dbconn = db[settings.MONGO_DB]
    ...
    if request.method == 'POST':
        try:
            stream = StringIO(request.body)
            data = JSONParser().parse(stream)
            classRoomBatch = dbconn.system_js.fnSaveClassRoomBatch(data)    
        except ValueError:
            return Response(json.dumps(ValueError, default=json_util.default))
        return Response(json.dumps(classRoomBatch, default=json_util.default))
    else:        
        return Response(json.dumps("failed", default=json_util.default))
...
@csrf_exempt
@api_view(['GET','POST'])
def loadClassRoomBatchView(request):  #this service will save class room batch details
    #connect to our local mongodb
    db = Connection(settings.MONGO_SERVER_ADDR,settings.MONGO_PORT)
    #get a connection to our database
    dbconn = db[settings.MONGO_DB]
    ...
    if request.method == 'POST':
        try:
            stream = StringIO(request.body)
            data = JSONParser().parse(stream)
            classRoomBatch = dbconn.system_js.fnLoadClassRoomBatch(data)    
        except ValueError:
            return Response(json.dumps(ValueError, default=json_util.default))
        return Response(json.dumps(classRoomBatch, default=json_util.default))
    else:        
